...
describe('AsyncButtonTwoRequestsOnClick', () => {
  let page;
  beforeAll(async () => {
    page = await global.browser.newPage();
    await page.goto('http://localhost:8001/asyncButtonTwoRequestsOnClick');
  }, timeout);

  afterAll(async () => {
    if (process.env.JEST_UPDATE_DATA_ENV) {
      await updateTestData(page, 'AsyncButtonTwoRequestsOnClick');
    }
    await page.close();
  });
